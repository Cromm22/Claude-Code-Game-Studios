...
> **Last Updated**: 2026-06-08 (round-7 Session 2 reconciliation-tick lifecycle patch applied)
...
3. **Squad oxygen cost**: 1 shared-oxygen-pool unit (exact unit defined by Resource Management GDD). Cost is triggered by **two server-side paths** that converge on the same idempotent T5 death procedure:
   ...
   **Idempotency**: T5 maintains two per-player booleans, `_deathCostPaid` and `deathDeductionCommitted` (both init `false` at session start / `Players.PlayerAdded` per R6-I11, **AND both cleared back to `false` on every T6 (respawn) transition** — see T6 side-effects in the C.6 transition table). Both paths check `_deathCostPaid` before acting; the reconciliation tick checks `deathDeductionCommitted` to decide whether a row still owes a deduction. **Without the T6 clearance of BOTH flags, the second death of a session pays zero squad oxygen** — on the first death, `_deathCostPaid` and `deathDeductionCommitted` are set `true`; if only `_deathCostPaid` were cleared on respawn (the R6-B5 defect), the second life's failed-then-rolled-back T5 would leave `_deathCostPaid = false` but `deathDeductionCommitted` still `true` from life 1, so the reconcile-eligible predicate (`_deathCostPaid == false AND deathDeductionCommitted ~= true`) evaluates **false** and the tick never recovers the lost deduction — the squad cost for life 2 is permanently lost (same defect class as round-4 B6, reproduced for the new flag). **Clearing both flags on T6 is load-bearing for Pillar 2 across multi-life squad runs.**
   ...
   **Set-before-yield discipline (atomicity rule — load-bearing for the entire idempotency claim)**: Luau is cooperatively scheduled — any function that may yield (`RemoteFunction:InvokeServer`, `Signal:Wait`, `task.wait`, `pcall(:GetAsync)`, `:InvokeServer` round-trips, `Knit:GetService(...):Method()` if the service performs a remote call internally) creates a yield boundary across which other coroutines (including the OTHER path's handler) can run. The check-and-set on `_deathCostPaid` MUST therefore complete in a single non-yielding span. **The implementation order is mandatory**:
   ...
   5. **Failure-mode rollback path**: if `pcall` returned `(false, errMsg)` OR the RemoteFunction's resolved result indicates failure (per Resource Management's `RequestSquadOxygenSpend` contract — e.g., `{success = false, reason}`), **clear `_deathCostPaid` for this death event back to `false`** to roll back the flag, surface the failure to a server log channel, and **do NOT broadcast `OnPlayerDied`** for this attempt. The squad oxygen ledger is unchanged.

      **Log-name safety (R6-B10 — destroyed-instance crash)**: the failure log MUST NOT dereference `player.Name` directly. On a **Path B reconciliation retry**, the `Player` instance has been removed from the `Players` service and may be destroyed by tick time; `player.Name` then raises a *nil-parent runtime error*, which **crashes the reconciliation handler and kills every subsequent retry for the entire server session**. The death context therefore carries a **`playerName` string snapshotted at the moment the death event was created** (at `Humanoid.Died` for Path A, or as the first statement of the `PlayerRemoving` handler for Path B — alongside the `lastKnownPosition` and `userId` snapshots), and the log reads that snapshot, never the live instance: `warn("[PlayerController] RequestSquadOxygenSpend failed for " .. deathContext.playerName .. " (userId=" .. tostring(deathContext.userId) .. "): " .. tostring(errMsg))`. The same snapshot rule applies to any other log/telemetry line in the reconciliation path.
      ...
      **The reconciliation tick is the authoritative retry surface (R5-B2 option (a) — CD-recommended, user-confirmed 2026-05-04).** PlayerController owns a server-side **death-cost reconciliation tick** that runs every `RECONCILIATION_TICK_INTERVAL = 5.0 s` on `RunService.Heartbeat`.

      **Death-event identity and keying (R6-B9 — user-confirmed 2026-06-08)**: every death event — Path A (`Humanoid.Died`) or Path B (`PlayerRemoving`-while-imminent) — creates a `deathContext` at the moment it fires, stamped with a **unique `deathEventId` from a monotonic server-session counter** and snapshotting `{deathEventId, userId, playerName, lastKnownPosition}` (the snapshots taken before any yield; `playerName`/`lastKnownPosition` are load-bearing for R6-B10 log-name safety and V/A.3 Path-B Death-SFX anchoring respectively). Two keying domains exist and do not overlap:

      - **Path A (player still present — S4 Dead-Respawning):** the live per-player flags `_deathCostPaid[player]` and `deathDeductionCommitted[player]` remain **`Player`-instance-keyed** (consistent with F.4's `Player`-instance keying mandate). The `deathContext` is associated with the player but the working flags are read off the instance key.
      - **Path B (player removed):** the `Player` instance is gone, so the reconciliation row lives in a **`deathEventId`-keyed working-set** — `reconcileRows[deathEventId] = { userId, playerName, lastKnownPosition, _deathCostPaid = false, deathDeductionCommitted = false }`. There is **no re-keying step** and the row never indexes a destroyed `Player` instance.

      This resolves the round-6 dual-table contradiction: PC keys nothing in the reconciliation path by `userId` (the ambiguous identifier across rejoins). The tick scans all rows — both Path-A live-player flags and Path-B `reconcileRows` entries — whose state matches:

      ```
      reconcile_eligible(row) =
          (row is a Path-A player ∈ S4 (Dead-Respawning) OR row is a Path-B reconcileRows entry)
          AND row._deathCostPaid == false
          AND row.deathDeductionCommitted ~= true
      ```

      where `deathDeductionCommitted` is set to `true` ONLY at step 6's success path (after BOTH a successful `RequestSquadOxygenSpend` AND a successful `OnPlayerDied` broadcast).

      **Rejoin-during-reconciliation race (closed by deathEventId keying)**: if a Path-B player rejoins under the same `userId` while their prior death's `reconcileRows[deathEventId]` row is still pending, the rejoin is a **fresh `Player` instance with its own fresh `Player`-keyed flags** (`_deathCostPaid[newPlayer] = false`, `deathDeductionCommitted[newPlayer] = false`, both initialized on `Players.PlayerAdded` per R6-I11 below). The pending `deathEventId` row is keyed independently and is **not collided with, not overwritten, and not satisfied by** the rejoin — it reconciles (or fails-and-retries) on its own deathEventId. Exactly one squad-oxygen unit is charged for the original death event; the new life's eventual death is a separate deathEventId.

      **Per-retry execution off the Heartbeat handler (R6-I5)**: a reconciliation retry invokes `RequestSquadOxygenSpend`, which is a RemoteFunction round-trip (50–150 ms). With up to 4 eligible rows in a 4-player squad, running the retries inline on the Heartbeat callback would block the callback for 200–600 ms — falsifying H.28's per-frame budget. **Each eligible row's retry MUST be dispatched in its own `task.spawn` off the Heartbeat handler** (the tick's Heartbeat body only *scans and dispatches*; it never yields). The set-before-yield discipline still holds inside each spawned retry. A per-tick row is dispatched at most once per tick interval (a row already mid-retry from a prior tick is skipped — guard on an in-flight marker).

      The reconciliation tick re-invokes the T5 deduction procedure for any matching row with the same set-before-yield discipline, the same `pcall`, and the same rollback path on failure (which simply re-arms for the next reconciliation tick). **On success the tick (a) sets `deathDeductionCommitted = true`, (b) broadcasts `OnPlayerDied(playerId, deathCause="reconciliation-recovery")` so HUD / squad communication can distinguish a delayed death notification from a normal one, and (c) invokes the T5 phantom-emission-suppression side-effects per R6-B8 below.** For a Path-B row, success also removes the `reconcileRows[deathEventId]` entry. **The flag rollback at step 5 is therefore the handoff to the reconciliation tick, not the end of the recovery path.**

      **Reconciliation-success T5 side-effects (R6-B8 — phantom-emission closure for the rollback-then-reconcile path)**: when the *initial* T5 `pcall` failed BEFORE step 6's side-effect block was reached, the dying player's sprint/light pulse-interval timers were never stopped. A successful reconciliation MUST therefore re-invoke the **player-removal-safe subset** of the T5 side-effects — specifically **(a) sprint pulse-timer stop / force-T2 if S2 was active** and **(b) lantern force-T4 / Light pulse-timer stop if S3b was active** — and MUST **explicitly suppress (c)–(f)** (the stamina-freeze, grace-slate reset, spectator-camera activation, and client-input-lock side-effects are either already settled or inapplicable to a removed player). Without (a)/(b) on this path, a Path-B player who disconnected in S2/S3b before their initial deduction succeeded would continue emitting Sprint/Light pulses for up to `LIGHT_PULSE_INTERVAL = 4 s` on a `HumanoidRootPart` that is being destroyed — the **R5-B1 Pillar-1 phantom-emission class, reopened for the failure-then-reconciliation path**. (a) and (b) are idempotent: re-stopping an already-stopped timer is a no-op, so invoking them on both the initial T5 success path and the reconciliation path is safe.

      **Bound on retry latency**: a row whose initial deduction failed is recovered within at most `2 × RECONCILIATION_TICK_INTERVAL = 10.0 s` of the original death event (worst case: the tick fired immediately before the failure; the next tick observes the rollback state and retries successfully). For Path A players (Dead-Respawning), 10 s is well within the 30 s `RESPAWN_DELAY` window — the squad cost is reconciled before respawn arrival. For Path B rows, the entry persists in `reconcileRows` (keyed by `deathEventId`, never by the destroyed `Player`) until cleared on successful reconciliation OR at server shutdown.
      ...
      **Reconciliation tick lifecycle**:
      ...
      - **Per-player flag initialization (R6-I11)**: on `Players.PlayerAdded`, `PlayerControllerService` MUST initialize the live per-player flags `_deathCostPaid[player] = false` and `deathDeductionCommitted[player] = false` — **`false`, NOT `nil`** (mirrors the F.4 PredatorService init mandate). A `nil` flag would make the reconcile-eligible predicate's `== false` / `~= true` comparisons read against a missing entry and either crash or misclassify a never-died player as eligible. The `reconcileRows` working-set itself is bootstrap-initialized to `{}` before the first tick can fire.
      - **Per-tick scan**: O(active players + pending `reconcileRows` entries this session) — a single linear pass over the small per-server tables; <0.01 ms per tick at H.28's 4-player budget (the RemoteFunction retries themselves are `task.spawn`'d off the Heartbeat handler per R6-I5 and are not counted in the scan cost).
      - **Termination**: each Path-A flag-set is cleared from eligibility on success (deathDeductionCommitted set true); each Path-B `reconcileRows[deathEventId]` entry is removed from the working-set on success OR retained for the next tick on failure. No row is reconciled more than once per tick interval (in-flight marker guards against double-dispatch across ticks).
      - **Persistence boundary (R6-B11 F3h invariant)**: reconciliation state is in-memory per server session — it does NOT cross `BindToClose` or server-instance boundaries. A `reconcileRows` entry that is still pending when `BindToClose` fires is **lost** (the squad-oxygen decrement for that death event will not be applied this session). This is an accepted bound, not a silent defect: Resource Management owns the durable squad-oxygen ledger via ProfileStore, and `BindToClose` is a server-shutdown path where the squad run is already ending. Reconciliation is the in-session bridge between PC's death event and RM's persisted decrement, not a durable queue. AC F3h documents this invariant explicitly.
   ...
   6. **Success path**: if `pcall` returned `(true, ...)` AND the resolved result indicates success, broadcast `OnPlayerDied(...)`, **set `deathDeductionCommitted = true` on this death event's keying domain** — `deathDeductionCommitted[player]` for Path A (player present), or `reconcileRows[deathEventId].deathDeductionCommitted` for Path B (and remove the row once committed) — which anchors the reconciliation tick's "already committed" exit per step 5, and **invoke the full T5 transition side-effects per the C.6 T5 row**, which is normative for both Path A and Path B and includes:
...
| T6 | Respawn timer = 30 s elapsed AND squad oxygen pool ≥ 1 | S4 | S1 (with S3a) | Server: respawn at level-flagged anchor; stamina = `STAMINA_MAX`; lantern = `LanternLowered`; sprint state cleared; emit no disturbance; camera returns to player; **clear BOTH `_deathCostPaid[player] = false` AND `deathDeductionCommitted[player] = false`** (R6-B5 — load-bearing for Pillar 2: clearing only `_deathCostPaid` leaves `deathDeductionCommitted` stuck `true` from the prior life, so a failed-then-rolled-back second-life T5 is never recovered by the reconciliation tick and the second death pays zero squad oxygen — see C.5.3 idempotency prose); also clear `lastPredatorDamageTimestamp[player]` per F.4 T6 obligation |
...
| `OnPlayerDied` | S → C (squad + dead) | `{playerId, deathTimestamp, deathCause}` — `deathCause` is an enumerated string ∈ `{"died"` (normal `Humanoid.Died`, Path A)`, "disconnect-while-damaged"` (Path B imminent-death disconnect)`, "reconciliation-recovery"` (delayed broadcast from the death-cost reconciliation tick, R6-I4)`}` | On death only | n/a |
...
| `RECONCILIATION_TICK_INTERVAL` | 5.0 s | 2–10 | Gate | C.5.3, C.11 | Tick overhead grows; reconciliation thrash on transient RM failures | The 2× recovery bound (`2 × interval`) exceeds `RESPAWN_DELAY = 30 s`, so a Path-A player could respawn before the squad cost is reconciled (Pillar 2 silent gap). **Keep `2 × RECONCILIATION_TICK_INTERVAL < RESPAWN_DELAY`** |
...
- **H.35** (F3a) — AUTO-INTEGRATION — GIVEN a Path A player A who dies (`Humanoid.Died`) with `_deathCostPaid[A]=false`, AND a fault injected so the *initial* T5 `RequestSquadOxygenSpend` `pcall` returns `(false, err)` (flag rolled back to `false` per step 5; no `OnPlayerDied` broadcast; squad oxygen unchanged), `getServerTime()` injected under test control, WHEN the clock is advanced by `RECONCILIATION_TICK_INTERVAL = 5.0 s` to fire one reconciliation tick AND the fault is cleared so the retry succeeds, THEN the tick finds A reconcile-eligible (A ∈ S4, `_deathCostPaid[A]==false`, `deathDeductionCommitted[A]~=true`), re-invokes T5, squad oxygen decrements by exactly 1, `deathDeductionCommitted[A]` becomes `true`, and `OnPlayerDied(playerId=A, deathCause="reconciliation-recovery")` is broadcast. (Verifies the Path-A rollback→tick recovery handoff; B8 rollback is a handoff, not an endpoint.)
- **H.36** (F3b) — AUTO-INTEGRATION — GIVEN a Path B player B who disconnects while satisfying the imminent-death predicate, a `deathContext` created with `deathEventId = E` snapshotting `{userId, playerName, lastKnownPosition}` and `reconcileRows[E] = {_deathCostPaid=false, deathDeductionCommitted=false}`, AND a fault injected so the initial deduction rolls back, `getServerTime()` injected, WHEN the clock advances one tick interval and the retry succeeds, THEN the tick reconciles `reconcileRows[E]` (NOT keyed by any `Player` instance — B's instance is destroyed), squad oxygen decrements by exactly 1, `OnPlayerDied(deathCause="reconciliation-recovery")` is broadcast, and `reconcileRows[E]` is removed. No access to `B.Name` or `B.Character` occurs (the log path reads `deathContext.playerName`, R6-B10). (Verifies the deathEventId-keyed Path-B recovery + destroyed-instance safety.)
- **H.37** (F3c) — AUTO-INTEGRATION — GIVEN player A whose initial T5 actually *succeeded* (`deathDeductionCommitted[A]=true`, squad oxygen already decremented once), `getServerTime()` injected, WHEN two further reconciliation ticks fire, THEN A is NOT reconcile-eligible on either tick (the `deathDeductionCommitted ~= true` conjunct fails), `RequestSquadOxygenSpend` is NOT re-invoked, and squad oxygen is unchanged across both ticks. (Verifies the double-deduction guard — exactly one unit per death event regardless of tick count.)
- **H.38** (F3d) — AUTO-INTEGRATION — GIVEN a death event whose initial deduction fails and whose retry succeeds only on the *second* tick (worst case: a tick fired immediately before the failure), `getServerTime()` injected, WHEN the clock is advanced, THEN the deduction is committed no later than `2 × RECONCILIATION_TICK_INTERVAL = 10.0 s` after the original death event. (Verifies the Pillar-2 SLA bound; for a Path-A player, 10 s < `RESPAWN_DELAY = 30 s`, so the cost is reconciled before respawn arrival.)
- **H.39** (F3e) — AUTO-INTEGRATION — GIVEN a Path-B `reconcileRows[E]` entry that reconciles successfully on a tick, WHEN the success path runs, THEN `reconcileRows[E]` is removed from the working-set in the same tick AND a subsequent tick performs no work for `E` (the entry no longer exists). For a Path-A player, `deathDeductionCommitted[A]=true` removes A from eligibility identically. (Verifies the success-path exit / working-set termination.)
- **H.40** (F3f) — AUTO-INTEGRATION — GIVEN server start at injected time `T_0` with the reconciliation tick connected in `Knit:Start()` and `reconcileRows={}`, WHEN the injected clock has advanced less than `RECONCILIATION_TICK_INTERVAL` since `T_0`, THEN no reconciliation tick has executed a scan; the first scan executes only once the elapsed-time gate (`> RECONCILIATION_TICK_INTERVAL`, routed through `getServerTime()`) is satisfied. (Verifies the first-tick timing gate + bootstrap ordering.)
- **H.41** (F3g — binary signal for R6-B5) — AUTO-INTEGRATION — GIVEN a 4-player squad, player A dies life 1 (initial T5 succeeds: `_deathCostPaid[A]=true`, `deathDeductionCommitted[A]=true`, squad oxygen 5→4), T6 fires at `T_1 + 30 s` (A respawns), WHEN A dies a second time (life 2) and that second T5's deduction fails-then-rolls-back, AND a reconciliation tick fires, THEN A is reconcile-eligible on the tick — which is only possible if T6 cleared **both** flags to `false`. With the R6-B5 fix the tick recovers the life-2 deduction (squad oxygen 4→3, `OnPlayerDied(deathCause="reconciliation-recovery")`); without the fix, `deathDeductionCommitted[A]` would still be `true` from life 1, the predicate would evaluate `false`, and the life-2 cost would be permanently lost. (This AC is the binary pass/fail signal that T6 clears `deathDeductionCommitted`, not just `_deathCostPaid`.)
- **H.42** (F3h — documented invariant) — AUTO-INTEGRATION — GIVEN a pending `reconcileRows[E]` entry (initial deduction rolled back, not yet retried), WHEN `BindToClose` fires before the next reconciliation tick, THEN the entry is lost and no squad-oxygen decrement is applied for `E` this session. This is the **accepted in-session-bridge bound** (reconciliation state is in-memory, not a durable queue; RM owns the ProfileStore ledger and the run is ending at shutdown). (Verifies the documented persistence-boundary invariant, NOT a defect — asserts the bound is the one the spec claims.)
- **H.43** (R6-I12 exact-boundary) — AUTO-UNIT — GIVEN the imminent-death predicate with `getServerTime()` injected, WHEN evaluated at the exact boundary values `Humanoid.Health = 25.0`, `(now - lastPredatorDamageTimestamp) = 30.0 s` (arm 1), and `= 3.0 s` (arm 2), THEN all three strict-`<` comparators return `false` and the predicate does NOT fire (neither arm). Paired with off-boundary cases `24.99` / `29.99 s` / `2.99 s` (predicate fires) to exercise the live branch. (Verifies C.5.3 / G.6 strict-`<` boundary semantics — the "use 24.99 / 25.01" tuning guidance is encoded as an AC.)
