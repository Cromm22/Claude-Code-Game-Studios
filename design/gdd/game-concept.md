...
8. **Win condition: craft the Escape Beacon, activate it, and survive the ~49 s survival window.** Activation opens the window (it does not win instantly); the session ends in **victory** if ≥1 squad member is alive at window-end, or in **defeat** on a full-squad wipe before window-end. No alternative route. *(Updated 2026-06-01 to the survival-window model per the Crafting & Items round-2 redesign — see `design/gdd/crafting-and-items.md` C.5.6a.)*
