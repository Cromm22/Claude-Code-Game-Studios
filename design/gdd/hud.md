...
| **Predator AI** | `OnPredatorSense {bearing, distanceBand}` — **per-client targeted fire to EVERY alive client per tick** (PA FIX-17 fan-out contract; seam-patch 2026-07-05, cross-review C5: this is the bearing/band source — the prior attribution to Channel B was wrong; Channel B fires only to the locked player, so non-locked squadmates would never have received the chevron) | per-alive-client | distance-band/bearing chevron (YOU **and** SQUAD threat-edge states — CR.2/CR.8 consume this) |
| **Predator AI** | `OnPredatorLockChanged {locked, audioSignature, lockedPlayerId, attackCaption, telegraphCaption}` (Channel B — lock-specific fields only; bearing/band re-attributed to `OnPredatorSense` per the row above) | per-locked-player | predator-lock **YOU** + eye-shine/CONTACT treatment + attack/telegraph captions |
...
| Variable | Type | Range | Description |
|---|---|---|---|
| `bearing` | float | [0, 360) | world-space angle player→predator (PA-owned, **`OnPredatorSense` per-client fire — PA FIX-17**; seam-patch 2026-07-05 re-attribution from the erroneous `OnPredatorLockChanged`, which never carries bearing to non-locked clients) |
...
| **Predator AI** | `OnPredatorSense` (per-client bearing/band fire, PA FIX-17 — seam-patch 2026-07-05 addition, cross-review C5) + `OnPredatorLockChanged` (Channel B, lock-specific fields) + predator state-change broadcast | distance-band/bearing chevron (from `OnPredatorSense`); predator-lock YOU/SQUAD, eye-shine/CONTACT, attack/telegraph captions (Channel B); Patrol baseline (broadcast) |
...
- **warning `#FFF860`** — hue ≈ **57° (OUTSIDE the 18°–48° band by 9°)** at S ≈ **62%** → clears the §4.2 exclusion by hue alone; no disambiguation triad or AD sign-off required. **Resolved 2026-07-06**: the prior value `#FFD060` (hue ≈ 42°, inside the band) was found to be a genuine exclusion-zone violation, not a borderline case as an earlier note claimed ("~50°, 2° outside the bound" — that arithmetic was wrong); rather than carve an exception into §4.2, the Art Director recommended and the value was changed to `#FFF860`, which clears the band with a 24°-total-separation margin from the predator's reserved hue. This also retires the OQ.6(b) sign-off item below — no exception is needed anymore.
...
- **H.20 [S]** — GIVEN an `OnPredatorSense` payload (the per-client bearing/band fire, PA FIX-17 — seam-patch re-verify 2026-07-06 rewrote this AC's channel: the prior wording read the fields off `OnPredatorLockChanged`, which per the corrected Interactions table carries NO bearing/band), THEN it contains no `worldPosition` field and the HUD renders only `bearing` + `distanceBand` (no position reconstruction; RR-4); AND GIVEN an `OnPredatorLockChanged` (Channel B) payload, THEN it carries NO `bearing`, NO `distanceBand`, and NO `worldPosition` field (lock-specific fields only — a payload carrying any of the three FAILS this AC).
...
- **H.22 [L]** — GIVEN the client's latest `OnPredatorSense` read is `distanceBand="CONTACT"` WHILE an active `OnPredatorLockChanged {locked=true}` targets this player (seam-patch re-verify 2026-07-06 channel rewrite — `distanceBand` arrives on the per-client `OnPredatorSense` fire, not on Channel B), THEN the eye-shine/CONTACT treatment is active on the predator-lock indicator; GIVEN `"NEAR"` or `"FAR"`, THEN eye-shine is absent (and no world position is read).
...
| **OQ.4** | **`bearing` reference convention** (D.3) — the HUD assumes clockwise-from-north for `θ_screen`; confirm PA emits the same convention on `OnPredatorSense` (bearing's actual source since the seam-patch re-attribution — corrected here, was misreferenced to `OnPredatorLockChanged`). | Predator AI | Quick confirm with PA (a one-line contract check) |
...
| ID | Question | Owner | Target |
|---|---|---|---|
| **OQ.6** | **In-band warm hues needing AD exclusion-zone sign-off** (corrected arithmetic, VA.1/VA.3): **(a)** CONTACT cream `#FFF0C8` (hue ≈ 43.6° INSIDE the band, S ≈ 22% — passes only by the sat-gate) — now an **optional enhancement**, does NOT ship unless signed off; **the locked primary CONTACT treatment is the pure-UIStroke `4px→8px→4px` escalation** (no sign-off needed). ~~(b) Warning meter `#FFD060`...~~ **RESOLVED 2026-07-06**: the warning meter color was changed to `#FFF860` (hue ≈ 57°), which clears the exclusion band outright — no AD sign-off needed for this item anymore. Only (a) remains open. | art-director | Art-bible approval / asset-spec pass |
