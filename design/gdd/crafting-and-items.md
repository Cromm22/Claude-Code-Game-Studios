...
| BCT5 | Defensive decay-watch detects ED source expiry (`m(t, age) <= MAGNITUDE_FLOOR`); ~501 s after activation per ED.D.1 example | BC4 | BC6 | Server: remove Beacon BasePart from workspace if still present; fire `OnBeaconDecayed()` to squad; clear decay-watch timer. **Defensive fallback only** — fires ONLY from BC4 (the active window). BC5 is terminal-victory and BC6 is terminal-decay; neither is a valid `From` state (a transition out of BC5 would fire `OnBeaconDecayed` after victory, which E.26 forbids). Normally unreachable — the run ends at BCT4 victory or BCT-DEFEAT ~10× sooner. Was `BCT4` pre-round-2. |
...
- **THEN** `CRAFT_BASE_DWELL_TIME` for each recipe matches the locked values in C.2 (Oxygen Canister 2 s, Dampener Coil 5 s, Signal Anchor 5 s, Squad Relay 6 s, Escape Beacon 10 s — the 5-recipe round-2 catalog); the cosmetic skin's data contains no override for craft time; verified by a pre-ship cosmetic review checklist gate
