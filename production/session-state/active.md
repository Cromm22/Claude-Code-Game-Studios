...
## ED round-21 close-out — APPLIED 2026-06-19 (NOT a verdict; ED stays NEEDS REVISION)
Applied the batched ED-internal round-21 items to `design/gdd/ecological-disturbance.md`: (1) **`OnDisturbanceBandCrossed` AC = H.28d** (Cross-System Wiring, after H.28c; (a) one-fire-per-transition+direction, (b) ±0.03 hysteresis no-chatter, (c) no Service.Client/no-forge) + C.3.3 "AC owed"→"AC LANDED" flip; (2) **I-GD18-1** D.6 `bestScore` NaN/+Inf guard (parallel to E.16); (3) **R16-I-8** Overview Pillar-4 session-scoped sentence. **R16-I-7 + R16-I-11 = CLOSED-REDUNDANT** (C.1.6 already backs the grid for GetHottestHotspot; F.4 already has the Humanoid.Died + OnPlayerDied BindableEvent rows). AC-count reconciliation satisfied by unique `H.28d` (ED has no canonical AC tally). systems-index ED row "Open" note updated; review-log round-21 close-out entry appended. **Subagent gates (round-15 four-layer) NOT run — pre-verdict obligation; deltas tiny + self-contained.** **NEXT = fresh-session `/design-review design/gdd/ecological-disturbance.md` (the round-21 verdict) — DO NOT predict APPROVED.** Still open: deferred AC-extraction pass (embedded CI-YAML/fixtures) + the verdict. NOT yet committed.
