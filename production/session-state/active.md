...
Skeleton created at `design/gdd/hud.md` (8 required sections + Visual/Audio + UI + Open Questions). Lean review mode (CD-GDD-ALIGN skipped). HUD = systems-index #7 (Presentation/UI), MVP, depends on all 6 authored GDDs (terminal consumer, no downstream). Pillars 2 primary + 1 + 3. The ~279-ref obligation surface is consolidated in the prior assistant turn. Camera GDD (OQ.3) is the one undesigned dep → S4 spectator + respawn-timer HUD elements are provisional. **ALL 8 REQUIRED SECTIONS DONE: A (Overview) + B (Player Fantasy) + C (Detailed Design: CR.1–7 + States + Interactions) + D (Formulas D.1–D.7, systems-designer-validated) + E (Edge Cases) + F (Dependencies) + G (Tuning Knobs) + H (Acceptance Criteria H.1–H.29, qa-lead-validated — closed CR.5c + CR.7 coverage gaps).** **NEXT = the 3 remaining sections + Phase 5:** Visual/Audio (REQUIRED for UI — spawn art-director), UI Requirements (tap-targets/touch — UX flag; OQ.14 two-step touch friction), Open Questions (diegetic eye-shine fork; survivalWindowDuration source; bearing convention; Camera-GDD S4; RunController arbiter); then Phase 5 = registry candidates + systems-index → "Designed" + a fresh-session `/design-review design/gdd/hud.md`. **Resume: re-run `/design-system hud` in a fresh session — auto-detects A–H complete, resumes at Visual/Audio.** Locked: eye-shine = screen-space at `distanceBand==CONTACT`; S4 PROVISIONAL (Camera OQ.3); combined-flash WCAG ≤3/s HUD-owned; cue-precedence ladder (CR.4); 5-zone layout; beacon-window-as-overlay.
